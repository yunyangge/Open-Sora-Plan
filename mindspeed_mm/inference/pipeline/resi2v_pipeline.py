...
from mindspeed_mm.inference.pipeline.opensoraplan_pipeline import OpenSoraPlanPipeline

class ResI2VPipeline(OpenSoraPlanPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        start_frame = get_pixel_values([image_path])[0]
        start_frame = resize_transform(start_frame)
        start_frame = rearrange(start_frame, '1 c h w -> c 1 h w').unsqueeze(0)
        ...
        latents = self.prepare_latents(shape, generator=generator, device=device, dtype=prompt_embeds.dtype,
                                       latents=latents)
        latents_for_input = torch.cat([latents] * 2) if self.do_classifier_free_guidance else latents
        ...
        start_frame_latents = self.vae.encode(start_frame.to(self.vae.dtype))
        start_frame_latents = repeat(start_frame_latents, 'b c 1 h w -> b c t h w', t=latents.shape[2])
        start_frame_latents_for_input = torch.cat([start_frame_latents, torch.zeros_like(start_frame_latents)], dim=0) if self.do_classifier_free_guidance else start_frame_latents
        latents_for_input = torch.cat(latents_for_input, start_frame_latents_for_input, dim=1)

        latents = self.scheduler.sample(model=self.predict_model, shape=shape, latents=latents_for_input, model_kwargs=model_kwargs,
                                        extra_step_kwargs=extra_step_kwargs)

        latents = start_frame_latents + latents

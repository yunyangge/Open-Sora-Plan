...
class ResI2VScheduler:
    ...
    # 在start frame和video (或者delta) 之间插值，当sigma为1时interpolation为start frame，当sigma为0时interpolation为video (或者delta)，sigma从1到0
    def interpolate(
        self,
        start_frame: torch.FloatTensor,
        ...
        """
        sample_dtype = start_frame.dtype
        sigmas = sigmas.float()
        start_frame = start_frame.float()
        video = video.float()

        interpolation = sigmas * start_frame + (1.0 - sigmas) * video

        interpolation = interpolation.to(sample_dtype)

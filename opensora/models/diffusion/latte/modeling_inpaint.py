...
class OpenSoraInpaint(LatteT2V):
    def __init__(
        ...
        super().__init__(
            ...
            interpolation_scale_1d=interpolation_scale_1d,
        )
    ...
    def forward(
        ...
        # At inpainting mode, we need not to use image joint training.
        ...
        if attention_mask is None:
            attention_mask = torch.ones((input_batch_size, frame, h, w), device=hidden_states.device, dtype=hidden_states.dtype)
        ...
        if self.caption_projection is not None:
            batch_size = hidden_states.shape[0]
            encoder_hidden_states = self.caption_projection(encoder_hidden_states.to(self.dtype))  # B 1 L D -> B 1 L inner_D
            encoder_hidden_states_spatial = repeat(encoder_hidden_states, 'b 1 t d -> (b f) t d', f=frame).contiguous()

        # prepare timesteps for spatial and temporal block
        timestep_spatial = repeat(timestep, 'b d -> (b f) d', f=frame).contiguous()

...
from opensora.dataset.transform import get_params, maxhwresize, add_masking_notice, calculate_statistics, \
    add_aesthetic_notice_image, add_aesthetic_notice_video, MULTI_RATIOS, ONE_RATIO
...
class T2V_dataset(Dataset):
    def __init__(self, args, transform, temporal_sample, tokenizer_1, tokenizer_2):
        ...
        self.force_ratio = args.force_ratio
                ...
                if i.get('resolution', None) is None:
                    ...
                    if i['resolution'].get('height', None) is None or i['resolution'].get('width', None) is None:
                        ...
                        if not is_pick:
                            ...
                            continue
                        # compute sample_h and sample_w for training
                        if not self.force_resolution:
                            tr_h, tr_w = maxhwresize(height, width, self.max_hxw, ratios=self.ratios, hw_stride=self.hw_stride)
                            _, _, sample_h, sample_w = get_params(tr_h, tr_w, self.hw_stride, self.ratios)

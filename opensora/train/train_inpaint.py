...
def log_validation(args, model, vae, text_encoder, tokenizer, accelerator, weight_dtype, global_step):
    ...
    for prompt, init_image in zip(validation_prompt, validation_images):
        ...
        init_image = torch.from_numpy(np.copy(pil_to_array(init_image)))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, (x, attn_mask, input_ids, cond_mask) in enumerate(train_dataloader):
            with accelerator.accumulate(model):
                ...
                x = x.to(accelerator.device, dtype=ae.vae.dtype)  # B 3*C T H W
                ...
                x = x.to(weight_dtype)
                masked_x = masked_x.to(weight_dtype)

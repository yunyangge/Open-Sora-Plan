...
from opensora.sample.pipeline_i2v import Image2VideoPipeline
...
def save_video(video: torch.Tensor, save_path='output_video.mp4', fps=24):
    import cv2
...
def log_validation(args, model, vae, text_encoder, tokenizer, accelerator, weight_dtype, global_step):
    ...
    validation_dir = args.get("validation_dir", "./validation")
    ...
    save_dir = os.path.join(validation_dir, f"val_{global_step:09d}")
...
def main(args):
    ...
    if args.use_ema:
        ...
        ema_model = EMAModel(ema_model.parameters(), model_cls=OpenSoraInpaint, model_config=ema_model.config)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            if args.use_ema:
                load_model = EMAModel.from_pretrained(os.path.join(input_dir, "model_ema"), OpenSoraInpaint)
            ...
            for i in range(len(models)):
                ...
                load_model = OpenSoraInpaint.from_pretrained(input_dir, subfolder="model")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, (x, attn_mask, input_ids, cond_mask) in enumerate(train_dataloader):
            with accelerator.accumulate(model):
                ...
                x = x.to(accelerator.device, dtype=weight_dtype)  # B 3*C T H W
                ...
                with torch.no_grad():
                    ...
                    cond = torch.stack([text_enc(input_ids[i], cond_mask[i]) for i in range(B)])  # B 1 L D
                    
                    print(f"text embedding shape: {cond.shape}")
                ...
                if args.noise_offset:
                    ...
                    noise += args.noise_offset * torch.randn((x.shape[0], x.shape[1], 1, 1), device=x.device)

                bsz = x.shape[0]
                # Sample a random timestep for each image without bias.
                timesteps = torch.randint(0, noise_scheduler.config.num_train_timesteps, (bsz,), device=x.device)
                ...
                model_pred = model(
                    torch.cat([noisy_x, masked_x, mask], dim=1),
                    timesteps,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--text_drop_rate", type=float, default=0.1)
    ...
    # optimizer & scheduler
    parser.add_argument("--seed", type=int, default=42, help="Random seed that will be set at the beginning of training.")

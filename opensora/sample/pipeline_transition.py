...
class OpenSoraTransitionPipeline(OpenSoraPipeline):
    ...
    def __init__(
        ...
        transformer: OpenSoraTransition,
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                new_prompt_embeds = torch.cat([prompt_embeds, prompt_embeds, prompt_embeds], dim=0)
                new_prompt_attention_mask = torch.cat([prompt_attention_mask, prompt_attention_mask, prompt_attention_mask], dim=0)
                key_frame_edge = torch.zeros([2, 1, 1, 44, 80], dtype=torch.float16).to(latent_model_input.device)
                key_frame_idx=torch.tensor(45).to(latent_model_input.device)

                noise_pred = self.transformer(
                    latent_model_input,
                    attention_mask=attention_mask, 
                    encoder_hidden_states=new_prompt_embeds,
                    encoder_attention_mask=new_prompt_attention_mask,

...
def flow_matching_inversion(
    ...
    **kwargs,
    ...
    for step in tqdm(range(num_inverse_steps), desc="Inverting"):
    ...
    if resample:
        ...
        encoder_hidden_states = kwargs.get("target_encoder_hidden_states", encoder_hidden_states)
        encoder_attention_mask = kwargs.get("target_encoder_attention_mask", encoder_attention_mask)
        pooled_projections = kwargs.get("target_pooled_projections", pooled_projections)
        for step in tqdm(range(num_inverse_steps), desc="Resampling"):
            # expand the latents if we are doing classifier free guidance
            latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents
            step_index = num_inference_steps - num_inverse_steps + step
            timestep = (sigmas[step_index] * 1000).expand(latents.shape[0])
            noise_pred = model(
                latents,
                attention_mask=attention_mask,
                encoder_hidden_states=encoder_hidden_states,
